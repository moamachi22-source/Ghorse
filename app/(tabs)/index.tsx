...
import { addDose, getDoseByScheduledTime, markDoseTaken } from '@/database/doses';
import { getAdherenceStats } from '@/database/doses';
import { scheduleMedicationNotifications, scheduleLowStockNotification } from '@/notifications/scheduler';
import { toPersianDigits, formatTime } from '@/utils/persian';
import { isLowStock, getNextDoseTime } from '@/utils/calculations';
...
export default function HomeScreen() {
  ...
  const checkLowStockAlerts = async () => {
    for (const med of medications) {
      ...
      const taken = takenDosesMap[med.id] ?? 0;
  ...
  const handleMarkTaken = (medication: Medication) => {
    ...
    const existing = getDoseByScheduledTime(
      medication.id,
  ...
  const getTodayDoseInfo = () => {
    ...
    const taken = Object.values(takenDosesMap).reduce(
      (sum, val) => sum + val,
      0
    );
    return { total, taken };
  ...
  const renderEmpty = () => (
    <Animated.View entering={FadeIn.duration(600)} style={styles.emptyContainer}>
      ...
      <Text style={styles.emptySubtitle}>
        روی دکمه + بزنید تا اولین دارو رو اضافه کنید
      </Text>
    </Animated.View>
  );
        ...
        >
          <View style={styles.summaryItem}>
            <Text style={styles.summaryValue}>
              {toPersianDigits(medications.length)}
            </Text>
